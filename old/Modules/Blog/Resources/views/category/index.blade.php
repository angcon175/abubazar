                                ...
                                @forelse ($categories as $key => $category)
                                    <tr id="item_id{{ $category->id }}">
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('module.postcategory.index', $category->slug) }}" class="btn bg-info mr-1">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            @endif
                                            @if (userCan('postcategory.delete'))
                                                <form action="{{ route('module.postcategory.destroy', $category->id) }}"
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @if ($edit_category && userCan('postcategory.update'))
                            <form class="form-horizontal" action="{{ route('module.postcategory.update', $edit_category->slug) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                               ...
                               {{-- <div class="form-group row">
                                ...
                                <div class="form-group row">
                                    <div class="offset-sm-3 col-sm-12">
                                        ...
                                        <a href="{{ route('module.postcategory.index') }}" class="btn btn-danger mt-2"><i class="fas fa-times"></i>&nbsp; {{ __('cancel') }}</a>
                                    </div>
                                </div>
                            </form>
                        @elseif (!$edit_category && userCan('postcategory.create'))
                            <form class="form-horizontal" action="{{ route('module.postcategory.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group row">
                                    <x-forms.label name="name" required="true" class="col-sm-3" />
                                    <div class="col-sm-9">
